...
CCamera::CCamera()
{
	...
	Position = F3dVector (0.0f, 0.0f, 5.0f);
	ViewDir = F3dVector (0.0f, 0.0f, -1.0f);
	RightVector = F3dVector (1.0f, 0.0f, 0.0f);
...
void CCamera::RotateX (GLfloat Angle)
{
	...
	ViewDir = Normalize3dVector(ViewDir*cosf(Angle*float(PIdiv180))
								+ UpVector*sinf(Angle*float(PIdiv180)));
...
void CCamera::RotateZ (GLfloat Angle)
{
	...
	//Rotate viewdir around the right vector:
	RightVector = Normalize3dVector(RightVector*cos(Angle*float(PIdiv180))
